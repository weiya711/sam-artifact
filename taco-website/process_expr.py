...
primitives = ["lvlscan", "union", "inter", "repeat", "lvlwr", "add", "mul", "crddrop", "reduce", "order"]

# Parses the input log file of expressions
def parse_file(args):
...
# Helper function to permute in place
def permute(A, P, n):
...
def gen_crddrop_ast(expr, dictionary):
    [lhs, rhs] = expr.split("=")
    ...
    tree = ast.parse(rhs)
    analyzer = CrdDropAnalyzer(dictionary)
    result = analyzer.visit(tree)
    return analyzer.needs_crddrop()
...
class CrdDropAnalyzer(ast.NodeVisitor):
...
def parse_all(line_args):
    expr = line_args[0]
    result = parse_tensors(expr)
...
def uniqueify(lines, unique_fmt=True):
    ...
    for line in lines:
        if unique_fmt: 
            # Unique expressions and format combinations 
...
def find_max_order(dictionary, order=4):
    count = 0
    avg = 0
    for d in dictionary:
        ...
        if len(max_order) > order:
            count += 1
            avg += len(max_order)
    print(avg / float(count))
    return count
...
def find_expr(lines, primitive, is_dense=False, debug=False):
    ...
    if primitive == "lvlscan":
        ...
        for i,line in enumerate(lines):
            try:
                dictionary = parse_all(line)
                needs_crddrop = gen_crddrop_ast(line[0], dictionary)
                if debug:
                    if needs_crddrop:
                        print("CRDDROP:", i, line)
                    else:
                        print(i, line)
                count += int(needs_crddrop)
            except:
                print(i, "FAILED")
                assert line[0] == '"' or '"struggle=ZGllKEBtZ' in line[0], line[0]
                failed_lines.append(i)
        ...
        for i,line in enumerate(lines):
            try:
                ...
                assert line[0] == '"' or '"struggle=ZGllKEBtZ' in line[0], line[0]
                failed_lines.append(i)
        count = find_max_order(dictionary)
...
if __name__ == "__main__":
    ...
    if args.primitive == "all":
        ...
        # Counts for lvlscan and lvlwr are inverted
        # Since they count the algorithms (or expressions) that are still
        # representable (NOT lost) with the given sam primitive removed
        if primitive == "lvlscan" or primitive == "lvlwr":
            count = total - count 
